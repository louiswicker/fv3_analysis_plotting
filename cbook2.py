...
def GetModelTime(file_obj, DateTime, index=False, closest=False):
    ...
    tarray    = file_obj.variables['Times'][:]
    init_time = file_obj.START_DATE
    init      = datetime.datetime.strptime(init_time, '%Y-%m-%d_%H:%M:%S')                 # create a datetime object
...
def GetTimeIndex(file_obj, time, closest=False):
    """ file_obj is a netCDF data set pointer , time is the model time in secs
        Returns the index of the time coordinate for a given model time"""

    init_DT       = datetime.datetime.strptime(file_obj.START_DATE, '%Y-%m-%d_%H:%M:%S')   

    time_array    = netCDF4.chartostring(file_obj.variables['Times'][:])

    DateTimeArray  = [datetime.datetime.strptime(s, '%Y-%m-%d_%H:%M:%S') for s in time_array ]

    seconds_array  = [(s - init_DT).seconds for s in DateTimeArray]

    index = N.where(N.array(seconds_array) >= time)
    ...
    if closest:
            ...
            if N.size(index2) !=0 and N.size(index) !=0:
                ...
                if diff2 < diff1:
                    return index2[0][-1], DateTimeArray[index[0][-1]]
    else:
       if N.size(index) !=0:
           return index[0][0], DateTimeArray[index[0][0]]
...
def GetFileDateTime(file_obj, time, string=True):
    ...
    try:
        ...
        valid_DT = datetime.datetime.strptime(valid_time, '%Y-%m-%d_%H:%M:%S') + datetime.datetime(1900,1,1,0,0,0)
    except:
...
#===============================================================================
def ComputeWZ(x, y, u, v, file_obj=None):
...
def GetCoords(file_obj, XC='XC', YC='YC', XE='XE', YE='YE', ZC='ZC', ZE='ZE', tindex=None):
    ...
    nx = len(file_obj.dimensions['west_east'])
    ny = len(file_obj.dimensions['south_north'])
    nz = len(file_obj.dimensions['bottom_top'])
    xc = 0.5*(_dx) + _dx*N.arange(nx)
    yc = 0.5*(_dy) + _dy*N.arange(ny)
    xe = _dx*N.arange(nx+1)
    ye = _dy*N.arange(ny+1)
    zc = N.arange(nz)
    ze = N.arange(nz+1)

    try:

        zc = file_obj.variables["zw"][0,0:nz,:,:]

    except:

        ph   = file_obj.variables["PH"][0,0:nz+2,:,:]
        phb  = file_obj.variables["PHB"][0,0:nz+2,:,:]
        ze   = (ph+phb)/9.81
    ...
    return ((xc,yc,zc[:,0,0]),(xe,ye,ze[:,0,0]))
